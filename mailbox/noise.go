...
const (
	...
	// lengthHeaderSizeV0 is the number of bytes used to prefix encode the
	// length of a message payload.
	lengthHeaderSizeV0 = 2

	lengthHeaderSize = 4

	// encHeaderSizeV0 is the number of bytes required to hold an encrypted
	// header and it's MAC.
	encHeaderSizeV0 = lengthHeaderSizeV0 + macSize
...
type Machine struct {
	...
	// nextCipherHeaderV0 is a static buffer that we'll use to read in the
	// next ciphertext header from the wire. The header is a 2 byte length
	// (of the next ciphertext), followed by a 16 byte MAC.
	nextCipherHeaderV0 [encHeaderSizeV0]byte
...
const (
	// HandshakeVersion0 is the handshake version where the authData is
	// sent in act two.
	...
	// MinHandshakeVersion and MaxHandshakeVersion will cause the
...
func (b *Machine) RecvActOne(actOne [ActOneSize]byte) error {
	...
	b.handshakeVersion = b.maxHandshakeVersion
...
func (b *Machine) WriteMessage(p []byte) error {
	...
	// integer for handshake version 0 and within a 32-bit unsigned integer
	// for all other versions.
	if len(p) > math.MaxUint32 || b.handshakeVersion == HandshakeVersion0 &&
		len(p) > math.MaxUint16 {
	...
	var pktLen []byte
	if b.handshakeVersion == HandshakeVersion0 {
...
func (b *Machine) ReadHeader(r io.Reader) (uint32, error) {
	if b.handshakeVersion == HandshakeVersion0 {
		_, err := io.ReadFull(r, b.nextCipherHeaderV0[:])
		if err != nil {
			return 0, err
		}
	...
	// Compute the packet length that we will need to read off the wire.
	pktLen := uint32(binary.BigEndian.Uint32(pktLenBytes)) + macSize
	return pktLen, nil
}
